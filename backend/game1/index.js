...
app.post('/user_C_says', async function (req, res) {
    ...
    if (list_len >= 5){ 
        ...
        console.log(results.whichPlayerByB, results.confidenceLevelByB, results.rationaleByB);
        ...
        res.json({ 
            ...
            "whichPlayerByB":results.whichPlayerByB, "confidenceLevelByB":results.confidenceLevelByB, "rationaleByB":results.rationaleByB,
        });
        ...
        res.json({ 
            ...
            "whichPlayerByB":"", "confidenceLevelByB":"", "rationaleByB":"",
        });
    }
...
async function BGuesses(thread_B_id) {
    
    let {all_messages} = await retrieve_all_messages(thread_B_id);
    
    const response = await openai.chat.completions.create({
        model: model_for_B,
        response_format:{ type: "json_object" },
        messages: [
            {"role": "system", "content": "In our ongoing digital mafia game, you're known as Player B. Reflecting on the interactions to date, can you identify which player—Player A or Player C—is more likely to be a human? Please specify your choice by saying either 'Player A' or 'Player C,' and express your level of confidence in this decision by providing a percentage (e.g., '70%'). Also, please share your reasoning behind this assessment. Make sure you put the results in JSON format with the following keys:'which_player','confidence_level','rationale'"},
            {"role": "user", "content": all_messages}
        ]
    });
    ...
    const whichPlayerByB = respJson['which_player'].slice(-1);
    const confidenceLevelByB = parseFloat(respJson['confidence_level'].slice(0, -1)) / 100;
    const rationaleByB = respJson['rationale'];

    return [whichPlayerByB, confidenceLevelByB, rationaleByB];
}

module.exports.handler = serverless(app); // For AWS LAMBDA functions
// app.listen(3000); // for localhost
